...
class TwentyOneGame(ActionGuardMixin, Game):
    ...
    def _action_modifier_guide(self, player: Player, action_id: str) -> None:
        ...
        for modifier_id in MODIFIER_HELP:
            ...
            user.speak_l(
                "twentyone-change-card-guide-entry", "table", name=name, description=description
            )
    ...
    def _action_read_current_hand(self, player: Player, action_id: str) -> None:
        ...
        user.speak_l(
            "twentyone-read-hand-response", "table", hand=hand_text, total=self._hand_total(p)
        )
    ...
    def _action_read_active_effects(self, player: Player, action_id: str) -> None:
        ...
        if not opponent:
            user.speak_l(
                "twentyone-active-effects-single", "table", player=p.name, effects=my_effects
            )
            return
